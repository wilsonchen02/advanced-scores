...
class ApplicationWebsiteLink():
    ...
    def __init__(self) -> None:
        ...
        self.authorization_url = (
            ...
            + f'{self.client_id}&response_type=token'
        )
...
class LoginWindow():
    ...
    def _create_description_box(self) -> None:
        ...
        m_box.showinfo(
            "Description",
            "This program assumes that the user has advanced scores enabled on"
            "Anilist and also has put in scores for said advanced scores. It"
            "will only update anime entries marked as COMPLETED, CURRENTLY "
            "WATCHING, and PAUSED."
        )
    ...
    def _create_token_frame(self) -> None:
        self._token_frame = ttk.LabelFrame(
            self._login_window, text="Access Token"
        )
    ...
    def _create_input_box(self) -> None:
        ...
        input_box = ttk.Entry(
            self._token_frame, textvariable=self._access_token
        )
    ...
    def _login_button_callback(self) -> None:
        ...
        try:
            ...
            response = requests.post(
                graphql_url, json={'query': query}, headers=header
                )
            self._status_code = response.status_code
            login_response_parsable = response.json()

            self.auth_username = login_response_parsable[
                "data"]["Viewer"]["name"]
...
class WeightsManager():
    ...
    _status_code: int
    _token: str
    _variables: TypedDict

    def __init__(
        self, token: str, advanced_scores: str, entries: ttk.Entry
    ) -> None:
    ...
    def weights_button_callback(self) -> None:
        try:
            ...
            # Parse through media query and do the math (round to nearest
            # hundredth)
            # Loop through lists (in this order): CURRENTLY WATCHING,
            ...
            for status_lists in self._advanced_scores[
                ...
                # Query complexity is limited, max cap is 500. Split into
    ...
    def _create_category_weights_list(self) -> bool:
        ...
        for raw_entry in self._entries:
            ...
            if self._is_entry_empty(entry):
                ...
                m_box.showerror(
                    "Error", f"Weights must be between 0 and 1 (inclusive)."
                )
                return False
            else:
                self._category_weights.append(float(entry))
        epsilon = 0.000001
        if abs(sum(self._category_weights) - 1) > epsilon:
            # Error check: total weight must be 1
            m_box.showerror(
                "Error", f"Total weight must be equal to 1.\n"
...
class WeightsWindow():
    ...
    def _create_category_frame(self) -> None:
        self._category_frame = ttk.LabelFrame(
            self._weights_window, text="Categories"
        )
        self._category_frame.grid(row=2, column=0, padx=10, pady=10)

    def _create_user_label(self) -> None:
        self._user_label = ttk.Label(
            self._weights_window, text=f"User: {self._auth_username}"
        )
    ...
    def _create_weights_description(self) -> None:
        self._weights_description = ttk.Label(
            self._weights_window,
            text="Please insert the weight of each scoring section, with the " + \
                "total weight = 1"
        )
    ...
    def _get_category_names(self) -> List[str]:
        ...
        return self._advanced_scores[
            "data"]["User"]["mediaListOptions"]["animeList"]["advancedScoring"]
    ...
    def _setup(self) -> None:
        ...
        self._weights_manager = WeightsManager(
            self._token, self._advanced_scores, self._entries
        )
